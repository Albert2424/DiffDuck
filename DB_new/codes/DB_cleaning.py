...
def clean_df(
    ...
    df = (
        df.groupby([gl_seq, gl_cid])[gl_affinity]
...
def sort_AB(df, threshold=0.1, out_A="data_A.csv", out_B="data_B.csv"):
    ...
    for lig in unique:
        ...
        slice = df[
            df["SMILES"] == lig
        ]  # obtain the slice of the dataframe that contains the ligand
        ratios = np.array(slice[f"{gl_affinity} mean"])[:, None] / np.array(
            slice[f"{gl_affinity} mean"]
        )  # kd_j/kd_k
        ...
        # mask = (
        #     ratios < threshold
        # )  # select only the proteins that have a ratio lower than threshold

        mask = np.where(np.tril(ratios, k=-1)!=0)
        ...
        df_out_a = pd.concat(
            [df_out_a if not df_out_a.empty else None, r_a], ignore_index=True
        )
        df_out_b = pd.concat(
            [df_out_b if not df_out_b.empty else None, r_b], ignore_index=True
        )
...
def drop_if_in_training(
    filename, df, AF=True, out_file="clean_data.csv", af_out="AF_input.csv"
):
    """
...
if __name__ == "__main__":
    ...
    if args.display_col_names:
        ...
        try:
            ...
            df = drop_if_in_training(
                file_drop, df
            )  # --> clean_data.csv can be used for creating the protein PDB files (i.e. AlphaFold)
        except:
            print(
                f"Skipping drop if in training since {file_drop} is not defined or is from a different dataset"
            )
        # Sort the values of the final database in a way that kdA < kdB
        dfa, dfb = sort_AB(
            df, threshold=thresh
        )  # --> data_A.csv, data_B.csv files can be paired to run the DiffDock simulation
        # print(dfa.columns)
