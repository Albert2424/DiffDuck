...
def plot_best_chain_guess(run_name, fold='all'):
    ...
    cmap1 = plt.get_cmap('cividis').copy()
        ...
        for j in range(data.shape[1]):
            if not np.isnan(xpos[i][j]):
                if data_aff.iloc[i,j] == 0:
                    plt.text(xpos[i][j]+0.3, ypos[i][j]-0.3, "   ", size=10,
    ...
    plt.text(xpos[data.shape[1]-1][data.shape[1]-1]+0.2, xpos[data.shape[1]-1][data.shape[1]-1]-data.shape[1]+1-0.3, f"{len(df[df['Affine']==1])}", size=15,
                        bbox=dict(boxstyle="round",
                                ec='black',
                                fc=c_list2[0],
                                ),
    ...
    plt.text(xpos[data.shape[1]-1][data.shape[1]-1]+0.2, xpos[data.shape[1]-1][data.shape[1]-1]-data.shape[1]+1, f"{len(df[df['Affine']==0])}", size=15,
                    bbox=dict(boxstyle="round",
                            ec='black',
                            fc=c_list2[1],
                            ),
