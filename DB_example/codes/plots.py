...
parser.add_argument('--counts', type=bool, help='Whether to plot or not the counts graph. Default is False', required=False, default=False) 
...
def plot_rel_res(dfa,dfb, threshold=0.1, filename=None):
    ...
    plt.legend(fontsize=35,markerscale=4.,handles=[a.legend_elements()[0][-1],a.legend_elements()[0][0]], labels=[f'Reliability > 75%: {correct}',f'Reliability < 75%: {failed}'])
