...
def grid_regression_tree(
    ...
    mp.plot_feature_importance(
        ...
        X_test=X_test,
        y_test=y_test,
        out_name="features_regression_tree",
    )
    return best_estimator
...
def grid_regression_forest(
    ...
    print(f" Max Depth: {best_parameters['max_depth']}")
    print(f" Bootstrap: {best_parameters['bootstrap']}")
    print(f" Min samples split: {best_parameters['min_samples_split']}")
    print(f" Warm Start: {best_parameters['warm_start']}")
    print(f" Accuracy: {ac:.3f}")
    # print(f" Precision: {precision_fin:.3f}")  # (TA/(TA+FA)+TB/(TB+FB)+TC/(TC+FC))/3
    # print(f" Recall: {recall_fin:.3f}")  # (TA/(TA+FB+FC)+TB/(TB+FA+FC)+TC/(TC+FB+FA))/3
    # print(f" F1: {2/(1/precision_fin+1/recall_fin):.3f}")
    ...
    mp.plot_feature_importance(
        ...
        X_test=X_test,
        y_test=y_test,
        out_name="features_regression_grid",
    )
    return best_estimator
...
if __name__ == "__main__":
    ...
    if args.labels == "ABC":
        ...
        best_tree = grid_tree(cv, clf_tree, args.data, grid_param)
        ...
        best_forest = grid_forest(args.data, cv, args.model_name, grid_param)
        ...
        best_tree = grid_regression_tree(cv, clf_tree, args.data,grid_param)
        ...
        clf_reg = grid_regression_forest(args.data,cv,args.model_name,grid_param)
    else:
